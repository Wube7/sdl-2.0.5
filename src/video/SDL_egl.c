...
#define DEFAULT_ALTERNATE_EGL "/opt/vc/lib/libbrcmEGL.so"
#define DEFAULT_OGL_ES2 "/opt/vc/lib/libGLESv2.so"
#define DEFAULT_ALTERNATE_OGL_ES2 "/opt/vc/lib/libbrcmGLESv2.so"
...
SDL_EGL_LoadLibrary(_THIS, const char *egl_path, NativeDisplayType native_display)
{
    ...
    if (egl_dll_handle == NULL) {
        if (_this->gl_config.profile_mask == SDL_GL_CONTEXT_PROFILE_ES) {
            if (_this->gl_config.major_version > 1) {
                ...
                egl_dll_handle = SDL_LoadObject(path);
                #ifdef DEFAULT_ALTERNATE_OGL_ES2
                if (egl_dll_handle == NULL) {
                    egl_dll_handle = SDL_LoadObject(DEFAULT_ALTERNATE_OGL_ES2);
                }
                #endif
            } else {
                path = DEFAULT_OGL_ES;
    ...
    if (dll_handle == NULL || SDL_LoadFunction(dll_handle, "eglChooseConfig") == NULL) {
        ...
        #ifdef DEFAULT_ALTERNATE_EGL
        if (dll_handle == NULL) {
            dll_handle = SDL_LoadObject(DEFAULT_ALTERNATE_EGL);
        }
        #endif
